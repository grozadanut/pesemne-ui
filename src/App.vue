...
const getRandomWords = (count) => {
  const shuffled = [...words.value].sort(() => 0.5 - Math.random())
  return shuffled.slice(0, count)
}

// Create reactive references for the game state
const choices = ref(getRandomWords(4))
const correctAnswer = ref(choices.value[Math.floor(Math.random() * choices.value.length)])
const selectedAnswer = ref(null)
const videoRef = ref(null)
const videoUrl = ref('https://pesemne.ro/wp-content/uploads/clips/'+correctAnswer.value.clip)
...
const checkAnswer = (selected) => {
  ...
  setTimeout(() => {
    choices.value = getRandomWords(4)
    selectedAnswer.value = null
    correctAnswer.value = choices.value[Math.floor(Math.random() * choices.value.length)]
    setVideoUrl('https://pesemne.ro/wp-content/uploads/clips/'+correctAnswer.value.clip)
  }, 3000)
}

// nou: funcție pentru a schimba URL-ul video-ului programatic
const setVideoUrl = (url) => {
  videoUrl.value = url
  // reîncarcă sursa videoclipului
  if (videoRef.value) {
    // setează src-ul direct și reîncarcă elementul
...
onMounted(() => {
  // încearcă să pornești playback programatic (va funcționa dacă videoclipul este muted/autoplay permis)
